...
function PartieAdministrateur({
  ...
  const addOption = () => {
    if (optionAAjouter != "") {
      const found = addOrEditQuestion.options.find(
        (option) => option == optionAAjouter
      );
      if (found == undefined) {
        const tabOptionTemp = [...addOrEditQuestion.options, optionAAjouter];
        setAddOrEditQuestion((prevState) => ({
          ...prevState,
          options: [...tabOptionTemp],
        }));
        setOptionAAjouter("");
      } else {
        alert("Existe déjà !");
  ...
  const sauvegarderModification = () => {
    if (
      ...
      addOrEditQuestion.bonne == "" ||
      addOrEditQuestion.options.length == 0
    ) {
      alert("Il faut remplir completement le formulaire");
    } else {
      const foundBonne = addOrEditQuestion.options.find(
        (option) => option == addOrEditQuestion.bonne
      );
      if (
        foundBonne != undefined &&
        addOrEditQuestion.options.length >= 2 &&
        ...
        alert(
          "Veuillez vérifier votre sélection. Il semble qu'il y ait une erreur ou que vous n'ayez pas sélectionné le bon nombre d'options."
        );
      }
    }
  };
  ...
  return (
    <div>
      ...
      {status === "add" || status === "edit" ? (
        <div>
          <div className="input">
            <label htmlFor="question">Insérez votre question</label>
            <input
              type="text"
              placeholder="Votre question"
              name="question"
              id="question"
              value={addOrEditQuestion.question}
              onChange={(e) =>
          ...
          <div className="input">
            <label htmlFor="options">
              Quelles sont les options que vous proposez
            </label>
            ...
            {addOrEditQuestion.options.length < 6 && (
              <div>
                <input
                  type="text"
                  placeholder="L'option"
                  name="options"
                  id="options"
                  value={optionAAjouter}
                  onChange={(e) => setOptionAAjouter(e.target.value)}
                />
          ...
          <div className="input">
            <label htmlFor="reponse">Quelle est la bonne réponse</label>
            <input
              type="text"
              placeholder="La réponse"
              name="reponse"
              id="reponse"
              value={addOrEditQuestion.bonne}
              onChange={(e) =>
                setAddOrEditQuestion({
                  ...addOrEditQuestion,
                  bonne: e.target.value,
                })
              }
            />
          </div>

          <Button handleClick={sauvegarderModification}>
            {status == "add" ? `Ajouter` : `Modifier`} une nouvelle question{" "}
        ...
        <>
          <Button handleClick={() => setStatus("add")}>
            Ajouter une nouvelle question <FontAwesomeIcon icon={faPlus} />
          </Button>
                  ...
                  {quizz.map((oneQuestion, index) => (
                    <tr key={`reponse-${index}`}>
                      ...
                      <td>
                        ...
                        <Button
                          quizz={quizz}
                          handleClick={() => deleteQuestion(index)}
                        >
                          <FontAwesomeIcon icon={faTrash} />
                        </Button>
                      </td>
          ...
          <Button
            handleClick={() => {
              setPartieAdministrateur(false);
            }}
          >
            Sortir de la partie administrateur
          </Button>
        </>
      )}
    </div>
